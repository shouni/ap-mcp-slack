...
- **Paginated listings never drop what they fetched.** `collectPages` in `slack.go` backs every listing (channels, joined channels, users). Slack's cursor resumes after a whole page, so a page's items are all kept even past `limit`; truncating a page while still returning its cursor would make the dropped items permanently unreachable. Route any new listing through it rather than hand-rolling a loop.

Failing to *read* something is generally not a failure of the operation. `chat.update`/`chat.delete` need neither a history scope nor `channels:read`, so `resolveTarget` degrades an unreadable message body or channel name into a note instead of an error; `ResolveUser` reports `search_truncated` rather than passing off a capped scan as a definitive `not_found`. Preserve this distinction when adding lookups: only an operation with no determinable target is an error.
