...
└── internal/
    ...
    │   ├── webapi.go    # webAPITransport（メッセージ投稿/削除・チャンネル一覧・履歴/スレッド取得）
    │   └── users.go     # ユーザー一覧・検索・解決
    ├── tools/           # MCP ツール定義
    │   └── slack.go     # Slackツールの入出力定義とハンドラ
...
| [slack-go/slack](https://github.com/slack-go/slack) | Slack Web API クライアント（chat.postMessage / chat.delete / conversations.list / users.conversations / conversations.history / conversations.replies / users.list / users.lookupByEmail） |
