...
`post_slack_message` / `post_slack_message_as_user` も `confirm` を省略/`false` にすると、実際には投稿せず上記と同じプレビュー情報（`posted: false` と合わせて）を返します。内容を確認した上で同じ入力に `confirm: true` を足して再実行すると、実際に投稿されます（`post_slack_message_as_user` の場合は `posted: true` と `ts` が返ります）。
